...
var findSumMatchRecursive = function findSumMatchRecursive(numbers, sum, firstIndex) {
  ...
  var solution = numbers.slice(0).reduce(function (acc, number, secondIndex, arr) {
    acc.firstIndex = firstIndex;

    if (arr[acc.firstIndex] !== number && arr[acc.firstIndex] + number === sum) {
  ...
  if (solution.secondIndex) {
    return "Matching indexes are: " + solution.firstIndex + "  and " + solution.secondIndex;
  } else {
...
var solution = findSumMatchRecursive([11, 2, 7, 15], 9);
